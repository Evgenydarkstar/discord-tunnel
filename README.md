...
6. сохранит клиентский CA-сертификат в `ca-cert.pem`.

После установки сохраните значения `Server`, `Port`, `Token` и передайте файл
`ca-cert.pem` на клиентский компьютер по безопасному каналу. Токен даёт доступ к
туннелю, поэтому не публикуйте его в репозитории, сообщениях или скриншотах.
...
7. В поле `CA Cert` выберите скопированный с сервера файл `ca-cert.pem`.
