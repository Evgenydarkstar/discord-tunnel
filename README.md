...
6. сохранит клиентский CA-сертификат в
   `<каталог-со-скачанным-deploy.sh>/.discord-tunnel/ca-cert.pem`.
...
запускались из домашнего каталога, сертификат нужно забрать по точному пути
`$HOME/.discord-tunnel/ca-cert.pem`. Чтобы вывести его абсолютный путь, выполните
из того же каталога, где лежит скачанный `deploy.sh`:
...
При таком способе сертификат находится в корне клона. Если `git clone` запускался
из домашнего каталога, точный путь — `$HOME/discord-tunnel/ca-cert.pem`. Из корня
клона абсолютный путь можно получить командой `realpath ca-cert.pem`.
...
7. Скопируйте с сервера `$HOME/.discord-tunnel/ca-cert.pem` (быстрая установка)
   или `$HOME/discord-tunnel/ca-cert.pem` (если клон создавался из домашнего каталога),
   затем нажмите `Browse` рядом с `CA Cert` и выберите этот файл.
